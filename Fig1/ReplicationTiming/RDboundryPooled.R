...
replicationTimePath = "../accesoryFiles/Data/UW_repliSeq/"
RDpath = "../accesoryFiles/Data/DNN_HMM_repliDomains/"
plotPath = "../plots/Fig1/"
RDconPath = "../accesoryFiles/Data/ConsTimingDomains"
...
fileNames <- list.files(RDpath)
cellName <- as.matrix(as.data.frame(strsplit(fileNames, "_"))[3,])[1:length(fileNames)]
RTdomain <- NULL
for(i in 1:length(fileNames)){
  cell <- read.table(paste(RDpath, fileNames[i], sep = ""),header = F,
...
for(cellType in 1:length(RTdomain)){
  ...
  RTdomains <- read.table(RDconPath, header = T)
...
earlyAnno <- boundaryAnotation(repList = joinRep[1:4], binSize = binSize, regionSize = regionSize, boundaryLine = TTReb, 
...
bigEBmeta <- NULL
files <- list.files(replicationTimePath)
for(i in 1:length(files)){
  bigEB <- import(paste(replicationTimePath, files[i], sep = ""), format = "bw", which = whichEB)
...
pdf(file = paste(plotPath,"PooledRtn.pdf", sep = ""), onefile = TRUE, width = 3.5, height = 10)
...
#### plotting things from beofre
 layout(matrix(c(3,2,1,4), nrow = 4))
  ...
  rotateTEmat <- NULL
for(TEs in 1:4){
  TE_choice <- names(joinRep[1:4])[TEs]
